...
```{r ans14}
# TpeJason
G5TimeC=ymd_hms("2020,Oct.9 21:00:00")
TpeJason<-
  with_tz(G5TimeC,tzone ="Asia/Taipei")
```
...
```{r ans15}
# ParisLouis
G5ParisTime<-with_tz(G5TimeC,tzone = "Europe/Paris")
ParisLouis= G5ParisTime+minutes(15)
...
# sumChNumbers
ch1to9 <- c("壹","貳","參","肆","伍","陸","柒","捌","玖")
f_ch1to9<-factor(ch1to9)
n_ch1to9<-as.numeric(f_ch1to9)
c_ch1to9<-as.character(n_ch1to9)
n_c_ch1to9<-as.numeric(c_ch1to9)
chNumbersss <- sample(n_c_ch1to9, 500, replace = T)
sumChNumbers<-sum(chNumbersss)
sumChNumbers
```
...
```{r ans22}
# sumChNumbersWith0
f_ch0to9<-factor(ch0to9)
n_ch0to9<-as.numeric(f_ch0to9)
c_ch0to9<-as.character(n_ch0to9)
n_c_ch0to9<-as.numeric(c_ch0to9)
chNumbersssWith0 <- sample(n_c_ch0to9, 500, replace = T)
sumChNumbersWith0<-sum(chNumbersssWith0)
sumChNumbersWith0
```
...
```{r ans32}
# dearStudent
dearStudent<-"Dear Jane:\n\n  貼心提醒您，作業上傳截止時間是2020-11-02 15:00:00 CST。\n\n敬祝學安\n\n你的貼心學伴巴西利bot silly"
